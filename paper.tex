...
strategy may be winning if for each \textsc{outcome} in the tree there exists a
play of controller and environment actions that is winning for the current
player. We can formulate this heuristic as a SAT query, which is constructed
recursively by $\textsc{treeFormula}$ ($\textsc{\textoverline{treeFormula}}$)
in Algorithm~\ref{alg:treeFormula}. The tree is first extended to the maximum
bound with \emph{unfixed} actions that allow existential choice by the SAT
solver. Then each game round in the tree is encoded in CNF by making copies of
the transition relation along with the winning condition of the current player.

The heuristic is based on the observation that for many synthesis problems in
the real world the final implementation will cooperate with its environment.
The existential search of the SAT solver simulates cooperation between the two
players.
