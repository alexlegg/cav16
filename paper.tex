        ...
        \State $\hat{T} \gets $ \Call{extend}{$T$} \Comment{Extend the tree with unfixed actions}
            ...
            \If{$sol = \texttt{unsat}$} 
                \If{\texttt{unbounded}} \Comment{Active only in the unbounded solver}
                    ...
                    \State \IfElse{$p = \texttt{cont}$}{\Call{learn}{$\sigma, \hat{T}$}}{\Call{\textoverline{learn}}{$\sigma, \hat{T}$}} \EndIfElse
                \EndIf
...
bound while learning states with interpolation over abstract games. We show
...
We modify the bounded synthesis algorithm to learn a subset of states from
failed attempts to find candidate strategies. When \textsc{findCandidate}
cannot find a strategy for an abstract game tree beginning at a state $s$, we
know that $s$ is a losing state. We can also attempt to learn additional losing
states from the tree via interpolation.
...
Consider the controller-losing game tree, $T$, in
Figure~\ref{fig:interpolatetree}. We choose any leaf node $l$ of $T$ and its
parent $n$. We then split the tree at $n$ such that both slices $T_1$ and $T_2$
contain $n$. Figure~\ref{fig:treef1} shows $T_1$, which contains all of $T$
except $l$, and $T_2$ (Figure~\ref{fig:treef2}) contains only $n$ and $l$ - a
single round of the game.  There is no candidate strategy for $T$ so $s \land
\textsc{treeFormula}(T)$ is unsatisfiable, and by extension $s \land
\textsc{treeFormula}(T_1) \land \textsc{treeFormula}(T_2)$ must also be
...
We can construct an interpolant with $F_1 = s \land \textsc{treeFormula}(T_1)$
and $F_2 = \textsc{treeFormula}(T_2)$. The only variables shared between $F_1$
and $F_2$ are the state variable copies belonging to node $n$. By construction
$F_2 \land \mathcal{I}$ is unsatisfiable, therefore all states in $\mathcal{I}$
must lose to the action in the edge between $n$ and $l$, the edge labelled by
action $b$ in Figure~\ref{fig:treef2}.  We also know that $F_1 \to
\mathcal{I}$, thus $\mathcal{I}$ contains all states reachable at $n$ by
following $T_1$ and avoiding the error state.
...
When we discover losing states for the controller we record them in a set of
states that \emph{must} be bad, $B^M$. This set is initialised to the error
state at the beginning of the algorithm and grows as losing states are
discovered.  The \textsc{treeFormula} function is modified for the unbounded
solver (see Algorithm~\ref{alg:unboundedTreeFormula}) to replace $E$ with
$B^M$. This enables further interpolants to be constructed by recursively
splitting more nodes from $T_1$ since the states that lost to $T_2$ are now
blocked by $B^M$.

It is more complicated to learn states from environment-losing abstract game
trees. States are only losing for the environment for a particular bound. For a
...
controller. The modified \textsc{\textoverline{treeFormula}} function
...
procedures. The unbounded algorithm performs bounded synthesis for iteratively
increasing maximum bounds until the initial state is in $B^M$ (environment
wins) or $B^m$ reaches a fixed point (controller wins). We prove correctness in
the next section.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Ensure $\sigma(X_T) \land B^m[$\Call{bound}{$T$}$] \equiv \bot$
...
\textoverline{\textsc{learn}}.  Below we prove that these functions indeed
maintain the invariants.

\subsection{Proof of learning functions}
...
removing $\II$ from $B^m[\textsc{bound}(n)]$ makes $\sigma(X_T) \land
...
\equiv B^m[\textsc{bound}(T)](X^T)$; hence $\sigma(X_T) \land
...
interpolant constructed from $F_1=\sigma(X_T) \land \textsc{treeFormula}(T_1)$
...
B^M(X_T)$.  Hence $\sigma(X_T) \land \textsc{treeFormula}(T_1) \equiv
\sigma(X_T) \land \lnot B^M(X_T) \equiv \bot$. Therefore $\sigma(X_T) \land
...
heuristic for minimising $\texttt{unsat}$ cores. $I$ is a value assignment to
each variable in $X$. If the environment does not win for $\langle I, k
\rangle$ then we attempt to solve for a generalised version of $I$ by removing
one assignment at a time. If the controller can win from the larger set of
states then we continue generalising without it. In this way we learn more
...
\begin{algorithm}
    \begin{algorithmic}
        \Function{checkBound}{$s, k$}
            \State $r \gets $ \Call{solveAbstract}{$\texttt{env}, s, k, \emptyset$}
...
results~\cite{syntcompedacc}, in Figure~\ref{fig:syntcomp}. The competition was run on Intel Quad
Core 32Ghz CPUs with 32GB of memory, also on isolated nodes for one hour per
instance.
...
\begin{figure}[h]
    ...
    \begin{tabular}{l l}
        \texttt{6s216rb0\_c0to31} & \texttt{driver\_c10n} \\
        \texttt{cnt30y} &  \texttt{stay18y} \\
        \texttt{driver\_a10n} & \texttt{stay20n} \\
        \texttt{driver\_a8n} & \texttt{stay20y} \\
        \texttt{driver\_b10y} & \texttt{stay22n} \\
        \texttt{driver\_b8y} & \texttt{stay22y} \\
    \end{tabular}
...
We also present a cactus plot of the number of instances solved over time. For
...
competes with a suite of 3 separate but communicating solvers. The solvers
...
extraction. The technique relies on interpolation of abstract game trees.  In
the present work we use interpolation in a similar way for computational
learning.
