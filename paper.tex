...
\subsection{Optimisation: Encouraging loops}

This optimisation is designed to encourage the system player to choose a
candidate strategy that contains loops. If the controller's strategy forces the
game into an error free loop, then the system wins the game. This optimisation
does not attempt to find such a strategy but instead optimistically searches
for candidate strategies that may loop. The modifications add formulas to the
SAT call when the controller is playing. These formulas represent loops of a
particular length in a branch of the tree. The SAT solver is called multiple
times for each branch in the game tree. The search begins by looking for a loop
in the first two game rounds of the branch and successively relaxing this
condition to longer loop lengths. If a loop is found or we reach the end of the
branch then the algorithm moves on to greedily find more loops in other
branches.
...
\begin{algorithm}
    \begin{algorithmic}
        \Function{loopFmls}{$T$}
            ...
            \For{$l \in leaves(gt)$}
                \State $j \gets 0$
                \For{$n \in$ \Call{parents}{$l$}}
                    \For{$p \in$ \Call{parents}{$n$}}
                        \State $loopFmls[i][j] \gets X_n \equiv X_p$
                    \EndFor
                    \State $j \gets j + 1$
                \EndFor
                \State $i \gets i + 1$
            \EndFor
            \State \Return $loopFmls$
        \EndFunction
    \end{algorithmic}
    ...
    \begin{algorithmic}
        ...
        \State $\hat{T} \gets $ \Call{extend}{$T$} \Comment{Extend the tree with unfixed actions}
            \State $f \gets $ \IfElse{$p = \texttt{cont}$}{\Call{treeFormula}{$\hat{T}$}}{\Call{\textoverline{treeFormula}}{$\hat{T}$}} \EndIfElse
            \If{$p = \texttt{cont}$}
                \State $loopFmls \gets $ \Call{loopFmls}{$\hat{T}$}
                \State $currentLoops \gets \top$
                \For{$i \in loopFmls$}
                    \For{$j \in loopFmls[i]$}
                        \State $sol \gets $ \Call{SAT}{$loopFmls[i][j] \land currentLoops \land s(X_{\hat{T}}) \land f$}
                        \If{$sol = \texttt{sat}$}
                            \State $currentLoops \gets currentLoops \land loopFmls[i][j]$
                            \State \textbf{break}
                        \EndIf
                    \EndFor
                \EndFor
