...
\begin{algorithm}
    \begin{algorithmic}[1]
        \Function{solveAbstract}{$\langle s, k \rangle , T$}
        \State $cand \gets $ \Call{findCandidate}{$\langle s, k \rangle, T$} \Comment{Look for a candidate}
        ...
        \Loop
            ...
            \State $\langle cex, l, u \rangle \gets $ \Call{verify}{$\langle s, k \rangle, T, cand$} \Comment{Verify candidate}
            ...
            \State $cand \gets $ \Call{solveAbstract}{$\langle s, k \rangle, T'$} \Comment{Solve refined AGT}
        \EndLoop
        \EndFunction
        \algstore{b1}
    \end{algorithmic}
    ...
    \begin{algorithmic}
        \algrestore{b1}
        \Function{findCandidate}{$\langle s, k \rangle, T$}
    ...
    \begin{algorithmic}
        \algrestore{b2}
        \Function{verify}{$\langle s, k\rangle, gt, cand$}
            \For{$l \in leaves(gt)$}
            \State $\langle k', s'\rangle \gets $ \Call{outcome}{$\langle s, k\rangle, gt, l$} \Comment{Get rank and state at leaf}
                \State $u \gets $ \Call{\textoverline{solveAbstract}}{$\langle k', s' \rangle, \emptyset$} \Comment{Solve for the opponent}
...
%%%            $\forall u_{k..1} \exists c_{k..1} \  s(x_k) \land \lnot E_k \land \delta(x_k, u_k, c_k) \land \lnot E(x_{k-1}) \land ... \land \delta(x_1, u_1, c_1) \land \lnot E(x_0)$
        \Function{checkRank}{$s, k$}
            \State \Return \Call{solveAbstract}{$\langle s, k \rangle, \emptyset$}
        \EndFunction
        \algstore{u2}
    \end{algorithmic}
