...
\author{Alexander Legg\inst{1} 
    ...
    \institute{Data61, CSIRO (formerly NICTA\thanks{NICTA is funded by the Australian Government as represented by the Department of Broadband,
...
The reactive synthesis problem is EXPTIME-complete (when starting 
from a symbolic representation of the game graph) so
na\"ive algorithms are infeasible on anything but simple systems.
...
it solves the largest number of unique instances, i.e.,
instances that could not be solved by any other sequential solver.  
These results confirm
that there exist classes of problems that are hard for traditional synthesis
techniques, but can be efficiently solved by our method.  While further
performance improvements are clearly needed, in its current state the method
may be a worthwhile addition to a portfolio solver.
...
controllable action variables $C$.  
We use $\mathcal{X}$, $\mathcal{U}$, and $\mathcal{C}$ to denote sets of valuations of 
variables $X$, $U$, and $C$ respectively.
$I$ is the initial state of the game given
...
A \emph{controller strategy} $\pi^c : \mathcal{X} \times \mathcal{U} \to \mathcal{C}$ 
is a mapping of states and
...
An \emph{environment strategy} $\pi^e : \mathcal{X} \to \mathcal{U}$ is a mapping of states to
            ...
            \node [circle,draw] (root){}
                child {node [circle,draw] {}
                    child {node [circle,draw] {}
                        ...
                        edge from parent [fixed] node [left] {\texttt{req=2}}
                    }
...
\begin{exmpI}
    ...
    its \emph{simplified version} to the running example.
    We begin by finding 
    a trace of length $k$ (here we consider $k=3$) that is winning for the environment, 
    i.e., that starts from the initial state and reaches an
    error state (see Figure~\ref{fig:trace}).  We use a SAT solver to find such a trace,
    precisely as one would do in bounded model checking.
    Given this trace we make an initial
    conjecture that any trace starting with action \texttt{req=1} is winning for the
    environment. This conjecture is captured in the abstract game tree shown in
    Figure~\ref{fig:agt}.  We validate this conjecture by searching for a counterexample
    trace that avoids the error state with the
    first environment action fixed to \texttt{req=1}.   
    Such a trace, that refutes the conjecture, is shown in Figure~\ref{fig:trace2}. 
    In this trace, the controller wins by responding with \texttt{gr0=1, gr1=1} in the first round.
    This move represents controller's partial strategy against the 
    abstract game tree in Figure~\ref{fig:agt}.  This partial strategy is shown in Figure~\ref{}.
    
    Next we strengthen the abstract game tree taking this partial strategy into account.
    To this end we again use a SAT solver to find a trace where the environment wins while
    the controller plays according to the partial strategy.  In the resulting trace (now shown), 
    the environment plays \texttt{req=2} in the second round.  We refine the abstract game tree
    using this move as shown in Figure~\ref{fig:agt2}.  The controller's partial strategy was to grant two
    resources in the first round, to which the environment responds by now
    making two requests in the second round.

    When the environment cannot refine the tree by extending existing branches, it backtracks 
    and creates new branches.
    Eventually, we obtain the abstract game tree shown in Figure~\ref{fig:refined} for which 
    there does not exist a winning partial strategy on behalf of the controller.  We conclude that 
    the game is winning for the environment.

%    Eventually, the refinements to the environment's candidate will force the
%    controller to grant more resources and result in a game tree such as in
%    Figure~\ref{fig:refined}.
...
(${|\mathcal{X}|}$) so that all states may be explored by the algorithm. A more nuanced
...
bound while learning bad states from abstract games using interpolation. We
            ...
            \node [circle,draw] (root){}
                child {node [circle,draw] {}
                    child {node [circle,draw,right=5pt] {}
                        edge from parent [fixed] node [left=2pt,text width=1cm] {\texttt{gr0=1 gr1=0}}
                    }
                    child {node [circle,draw,left=5pt] {}
                        edge from parent [fixed] node [right=2pt,text width=1cm] {\texttt{gr0=1 gr1=1}}
                    }
                    node [left=5pt] {$n$}
                    edge from parent [fixed] node [left=2pt,text width=1cm] {\texttt{gr0=0 gr1=1}}
                }
        ...
        \begin{tikzpicture}[dash pattern = on 2pt off 2pt, level distance = 10mm]
            \node [circle,draw] (root){}
                ...
                child {node [circle,draw] (n) {}
                    ...
                    edge from parent [fixed] node [right=2pt=2pt=2pt=2pt,text width=1cm] {\texttt{gr0=1 gr1=1}}
                }
                node [above=4pt] {$\langle s, k \rangle$};
        \end{tikzpicture}
...
memory.  The solver was allowed exclusive access to a node for one hour to
solve an instance.
...
for one hour per instance. The unique column lists the number of instances that
only that tool could solve in the competition (excluding our solver). In
brackets is the number of instances that only that tool could solve, including
our solver.
...
the only SAT-based tool in the competition, the winner of the sequential
realisability track \textsc{Simple BDD Solver 2}~\cite{walker2014}, and
AbsSynthe (seq3) \cite{brenguier2014}. Our results differ from the competition,
we suspect due to the weaker hardware.
