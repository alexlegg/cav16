...
state $s$.  We know that $s$ is a losing state.  We can learn additional losing
...
$\textsc{treeFormula}(T) = \textsc{treeFormula}(T_1) \land \textsc{treeFormula}(T_2)$
and hence $s \land
...
is discovered for a node $n$ with bound $\textsc{bound}(n)$ in line~13, Algorithm~\ref{alg:learn}, this set is subtracted from 
