...
al.~\cite{narodytska2014}, which is the main building block of our unbounded
algorithm.  The algorithm constructs a series of abstractions of the input
game.  
An abstraction restricts actions available to one of the players. 
%If the player can win the abstract game, then it can also win the original 
%concrete game; however the opposite is not generally true.
...
%Together with a state-round pair
...
%We identify an abstract game tree with the set of its nodes.  
...
$\langle s',k'\rangle=\textsc{outcome}(s, k, Strat, l)$, where $s'$ is the state obtained
by playing all controllable and uncontrollable actions on the path from the
root to the leaf, and $i'$ is the number of rounds left in the game after reaching the leaf.
An opponent's partial strategy is winning if for all its outcomes $\langle s',k'\rangle$,
$s'$ is a winning state for the opponent in the concrete game with bound $k'$.

%The abstract game tree restricts moves available to the player, thus making the game easier
%to win for the opponent.  Hence, if the opponent does not have a winning partial strategy in
%the abstract game, it also does not have a winning strategy in the concrete game.  
...
algorithm takes a concrete game $G$ with bound $n$ as an implicit argument. In addition, it
takes a player $p$ (controller or environment), state $s$, bound
$k$ and an abstract game tree $T$ and returns a winning partial strategy
for $opponent(p)$, if one exists.  The initial invocation of the algorithm takes the
initial state $\langle I, n\rangle$ and an empty abstract game tree
...
the algorithm uses the \textsc{findCandidate} function, described below, to come 
up with a candidate partial strategy for $T$. If it fails to
...
%{Figure~\ref{fig:spoiling}} shows a spoiling
