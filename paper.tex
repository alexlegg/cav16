...
hard for traditional synthesis techniques, but can be efficiently solved by our
method.  While further performance improvements are clearly needs, in its
current state the method may be a worthwhile addition to a portfolio solver.
...
$B^m[k]$ are initialised to $\top$ for all $k$.  Whenever an environment-losing
...
tells us that if $B^m[i] \equiv B^m[i+1]$ (line~6) then $Upre(B^m[i+1])
\subseteq B^m[i+1]$, i.e. there is a fixed point in the uncontrollable
predecessor. This guarantees that the controller can force the game to stay
within $B^m[i+1]$ and avoid error states indefinitely.
