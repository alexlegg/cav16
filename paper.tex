...
referred to as \emph{abstract game trees} (AGTs).  Figure~\ref{fig:agt} shows an
...
$n$, $\textsc{height}(n)$ is the maximal distance from n to a leaf. $\textsc{height}(T)$ is the height 
of the root node of the tree.  For node
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State \Return{$\lnot \Call{E}{X_{T}} \land$ \\
            $$\bigwedge_{\langle e, n \rangle \in \Call{succ}{T}}(\Call{$\delta$}{X_T, U_e, C_T, X_n} \land U_e = \Call{action}{e} \land \Call{treeFormula}{n})$$
        }
        \EndIf
        \EndFunction
        \algstore{tf1}
    ...
    \begin{algorithmic}[1]
        ...
        $$\bigvee_{\langle e, n \rangle \in \Call{succ}{T}}(\Call{$\delta$}{X_T, U_T, C_e, X_n} \land C_e = \Call{action}{e} \land \Call{\textoverline{treeFormula}}{n})$$ }
        \EndIf
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
For the system, this means the environment partial strategy represented by $T$
will always reach $E$ from $s$ no matter the assignments to $C$ variables (the
bound is irrelevant).  For the environment, the system can avoid $E$ for $k$
rounds no matter the assignments to $U$ variables. Hence, we have established
that $s$ is a losing state either always (for the system) or for bound $k$ (for
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \Ensure $s(X_T) \land B^m[$\Call{height}{$T$}$] \equiv \bot$
        \Comment{$s$ will be removed from $B^m[$\Call{height}{$T$}$]$}
...
$\textsc{treeFormula}(T) \equiv \textsc{treeFormula}(T_1) \land
\textsc{treeFormula}(T_2)$ and hence $s \land \textsc{treeFormula}(T_1) \land
\textsc{treeFormula}(T_2)$ is also unsatisfiable. This enables the construction
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \Else
        \State \Return{ \Call{$B^m[\Call{height}{T}]$}{$X_{T}$} $\land$ \\
            $$\bigg( \Call{E}{X_T} \lor \bigvee_{\langle e, n \rangle \in \Call{succ}{T}}(\Call{$\delta$}{X_T, U_n, C_e, X_n} \land C_e = \Call{action}{e} \land \Call{\textoverline{treeFormula}}{n})\bigg)$$ }
        \EndIf
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\textsc{\textoverline{treeFormula}}(T)$, which is unsatisfiable (precondition (b)).  Hence the
...
$B^m[\textsc{height}(n)-1]$ playing against the counterexample moves in $T_2$.
Hence, $\II \cap Upre(B^m[\textsc{height}(n)-1]) = \emptyset$.  Furthermore,
since the may-invariant holds, $\II \cap Upre(B^m[\textsc{height}(i)]) =
\emptyset$, for all $i < \textsc{height}(n)$.  Hence, removing $\II$ from all
$B^m[i], i\leq \textsc{height}(n)$ in line~(15) preserves the may-invariant,
...
\equiv B^m[\textsc{height}(T_1)](X^T)$; hence $s(X_T) \land
\textsc{\textoverline{treeFormula}}(T_1) \equiv s(X_T) \land
B^m[\textsc{height}(T_1)](X^T) \equiv \bot$.  Thus the second postcondition of
