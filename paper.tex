...
E(x_i))$.  In a \emph{bounded game} with bound $\kappa$ all runs are restricted
to length $\kappa$, whereas unbounded games consider runs of infinite length.
Since we consider only deterministic games, a run is uniquely described by a
list of assignments to $U$ and $C$.
...
in a bounded game of rank $n$ if all runs $(x_0, u_0, \pi^c(x_0, u_0)), (x_1,
u_1, \pi^c(x_1, u_1)) \dots$ $(x_n, u_n, \pi^c(x_n, u_n))$ are winning.
...
abstract game tree $T$ a \emph{partial strategy} $Strat: \textsc{nodes}(T) \rightarrow U$ 
($Strat: \textsc{nodes}(T) \rightarrow C$) labels each node of the tree with
...
in Figure~\ref{fig:extended}. Then copies of state and action variables are introduced for
each node in tree and opponent action variables for each edge.  Opponent
actions are set to $\textsc{action}(e)$, the action that labels the edge of the
tree $e$. The new state and action variables are connected to the successor
nodes (\textsc{succ}) by an encoding transition relation and constrained to the
winning condition of the player.
...
bound while learning states. We show that reaching a fixed point in learned
states is sufficient for completeness.
...
failed attempts to find candidate strategies.

When the abstract game is unsatisfiable this indicates that the states
represented by nodes in the abstract game tree are losing for the current
player. We extract these states from the game tree using interpolation.
...
Consider the game tree in Figure \ref{fig:interpolatetree}. The
