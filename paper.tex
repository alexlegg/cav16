...
Figure~\ref{fig:interpolatetree}, whose edges are labelled with controller
actions.  There does not exist a partial strategy for the environment that is
winning against the controller actions in this tree since any requests will
always be granted. Thus the \textsc{findCandidate} function can not find a
candidate strategy. We now learn the states for which these controller actions
can force the game to stay in the controller winning set.
...
\textsc{treeFormula}(k, T_2)$ and hence $s \land \textsc{treeFormula}(k, T_1)
\land \textsc{treeFormula}(k, T_2)$ is also unsatisfiable. This enables the
construction of an \emph{interpolant} that captures losing states at node $n$.
