...
it solves the largest number of unique instances for a sequential solver, i.e.,
instances that could not be solved by any other solver.  These results confirm
...
the variables (\ref{fig:examplevars}) and the formulas for computing next-state
variables (\ref{fig:exampletrans}) for this example. We use primed identifiers
...
$n$ resources. The set of winning states has no compact representation, which
...
\begin{exmpI}
    ...
    a trace of length $k$ (here we consider $k=3$) from the initial state to an
    error state (see Figure~\ref{fig:trace}).  Given a trace we make an initial
    conjecture that the first action, \texttt{req=1}, is winning for the
    environment. This conjecture is captured in the abstract game tree shown in
    Figure~\ref{fig:agt}.  Next the solver attempts to find a counterexample to
    this conjecture by finding a trace that avoids the error states with the
    first environment action fixed to \texttt{req=1}, i.e. by playing the
    abstract game on behalf of the controller.  
    
    The resulting trace is shown in Figure~\ref{fig:trace2}. The environment's
    candidate is refined using this trace and Figure~\ref{fig:agt2} shows the
    refined game tree.  The controller's counterexample was to grant two
...
\begin{exmpInt}
    ...
    employing interpolation instead. The potential benefit of interpolation is
    that an interpolant can approximate the losing states in such a way that
    there is just enough information to prove realisability. It is also cheap
    to compute an interpolant using a SAT solver.
\end{exmpInt}
